...
class SettingsDialog(gtk.Dialog):
  """Create a settings/preferences dialog for screenkeymon."""
...
def manually_run_dialog():
  ...
  import screen_key_mon

  SettingsDialog.register()
  gettext.install('screen-key_mon', 'locale')
  logging.basicConfig(
      level=logging.DEBUG,
      format = '%(filename)s [%(lineno)d]: %(levelname)s %(message)s')
  options = key_mon.create_options()
  options.read_ini_file('~/.config/screen-key-mon/config')
